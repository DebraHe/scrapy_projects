...
class ChyxxSpider(scrapy.Spider):
    ...
    def parse_content(self, response):
        time = response.css('div.detail div.info span:nth-child(1)::text').extract_first().strip()
        if time.split()[0] == datetime.date.today().strftime("%Y年%m月%d日"):
            item = ChyxxItem()
            item['datePublished'] = time

            item['headline'] = response.meta.get('title')
            item['Bigtype'] = response.meta.get('Bigtype')
            item['articleBody'] = response.css('div#contentBody').xpath('string(.)').extract_first().replace(
                u'中国产业信息网微信服务号', '').replace(u'中国产业信息网微信公众号', '').replace(
                response.css('div.content-info::text').extract_first(), '')
            item['annex'] = [urlparse.urljoin(response.url, each_pic) for each_pic in
                             response.css('div#contentBody img::attr(src)').extract()]
            try:
